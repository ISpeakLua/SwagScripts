--[[
	This script wasn't made by me. You can find the original one right here!
  https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source
  this repo will avoid updates that u dont want and will include some changes (that will be in future for now).
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'Infinite Yiff is waiting for the game to load'
	game.Loaded:Wait()
	notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "Infinite Yiff FE v"..ver
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Infinite Yiff Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if State == Enum.TeleportState.Started then
		if KeepInfYield and queueteleport then
			queueteleport(string.format("loadstring(game:HttpGet('%s'))()", githubrepo))
		end
	end
end)
